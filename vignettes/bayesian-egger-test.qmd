---
title: "Bayesian Egger's test"
vignette: >
  %\VignetteIndexEntry{bayesian-egger-test}
...
$$
where $\kappa$ is a multiplicative overdispersion factor. $\kappa = 1$ corresponds to the standard fixed-effect Egger model.

**Additive heterogeneity**:
$$
\varepsilon_i \sim \mathcal{N}(0, s_i^2 + \tau^2)
$$
where $\tau$ is the between-study heterogeneity, identical to the standard random-effects parameterisation.
...
For a visual complement to the Egger test, see [Funnel Plots](funnel-plots.html).
