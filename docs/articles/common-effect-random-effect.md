...
all approach zero together as $`\tau \to 0`$. The non-centred
parameterisation avoids this by sampling $`z`$ on a fixed
$`\mathcal{N}(0,1)`$ scale.
...
transitions in the centred parameterisation. The non-centred
parameterisation (used by default) resolves this in most cases. If
divergences persist:
