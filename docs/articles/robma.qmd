---
title: "Robust Bayesian Meta-Analysis (RoBMA)"
vignette: >
...
The two components share the mean $\mu$ but have different scales: $\tau$ for typical studies and $\tau_{\text{out}} = C \cdot \tau$ for outlier studies. The outlier component is much wider — it accommodates extreme effect sizes without pulling $\mu$ toward them.
...
The mixture parameterisation is generally well-behaved because both components share the same mean $\mu$, reducing label-switching risk. Use `adapt_delta = 0.95` and inspect per-chain trace plots for `pi_out` and `tau`.
...
Studies with `p_outlier > 0.5` are flagged as probable outliers in the summary table via `bayesma_output(fit_outlier, type = "outlier_probabilities")`.
