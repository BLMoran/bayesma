...
#' It supports both brmsfit objects (fitted via brms) and bayesma objects
...
#'   Required for brmsfit models. For bayesma models, this is extracted from the
#'   model object if not provided.
...
bayes_forest <- function(model,
  ...
  # ---- Column renaming (shared for both model types) ----
  ...
  # ---- Compute yi/vi for the data (needed by both pathways) ----
    es <- model$meta$es
    if (!is.null(es)) {
      data$yi <- es$yi
      data$vi <- es$sei^2
    } else {
      # Fallback: compute from forest_df if es not stored
  ...
  # This is used by study.density.plot_fn for the vertical reference lines
